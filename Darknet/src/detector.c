...
{
    char * log_file_url = "/home/yolo/project/train/log/log.txt";
	FILE *log_file = fopen(log_file_url,"w");
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
		...
		char add_info[1000];
		sprintf(add_info,"%d:  %f avg loss,  %f hours left,Last accuracy mAP@%0.2f = %2.2f %%, best = %2.2f %%\n",iteration,avg_loss,avg_time,iou_thresh, mean_average_precision * 100, best_map * 100);
		

		fwrite(add_info,strlen(add_info),1,log_file);
...
void calc_anchors(char *datacfg, int num_of_clusters, int width, int height, int show)
{
    ...
    FILE* fw = fopen("/home/yolo/project/train/src_repo/projects/anchors.txt", "wb");
    if (fw) {
